...
type ReplicaSetLister interface {
	...
	List(selector labels.Selector) (ret []*turtle.ReplicaSet, err error)
	// Get retrieves the ReplicaSet from the index for a given name.
	Get(name string) (*turtle.ReplicaSet, error)
...
// List lists all ReplicaSets in the indexer.
func (s *replicaSetLister) List(selector labels.Selector) (ret []*turtle.ReplicaSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*turtle.ReplicaSet))
	})
	return ret, err
}

// Get retrieves the ReplicaSet from the index for a given name.
func (s *replicaSetLister) Get(name string) (*turtle.ReplicaSet, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(turtle.Resource("replicaset"), name)
	}
	return obj.(*turtle.ReplicaSet), nil
}
