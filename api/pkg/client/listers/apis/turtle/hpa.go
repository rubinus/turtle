...
type HPALister interface {
	...
	List(selector labels.Selector) (ret []*turtle.HPA, err error)
	// Get retrieves the HPA from the index for a given name.
	Get(name string) (*turtle.HPA, error)
	HPAListerExpansion
}
...
// List lists all HPAs in the indexer.
func (s *hPALister) List(selector labels.Selector) (ret []*turtle.HPA, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*turtle.HPA))
	})
	return ret, err
}

// Get retrieves the HPA from the index for a given name.
func (s *hPALister) Get(name string) (*turtle.HPA, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(turtle.Resource("hpa"), name)
	}
	return obj.(*turtle.HPA), nil
}
