...
type ServiceLister interface {
	...
	List(selector labels.Selector) (ret []*turtle.Service, err error)
	// Get retrieves the Service from the index for a given name.
	Get(name string) (*turtle.Service, error)
...
// List lists all Services in the indexer.
func (s *serviceLister) List(selector labels.Selector) (ret []*turtle.Service, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*turtle.Service))
	})
	return ret, err
}

// Get retrieves the Service from the index for a given name.
func (s *serviceLister) Get(name string) (*turtle.Service, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(turtle.Resource("service"), name)
	}
	return obj.(*turtle.Service), nil
}
